...
```
YouTube-BB CSVs → filter by selected classes → download videos (yt-dlp, 7 threads)
→ extract frames at annotated timestamps (ffmpeg, 10 threads)
→ generate YOLO-format labels → remap class IDs to zero-indexed
→ split into train/val/test → cleanup
```
